...
emissions <- batch_extract("ANNUALEMISSIONS",all_gdx)[[1]] |> setDT() |> osemosys_sanitize()
{
  ...
  ggplot(emissions |> filter(EMISSION=="CO2")) +
    geom_line(aes(x=as.numeric(YEAR),y=value,color=scen), linewidth=1.5)+ 
...
prod2$value = round(as.numeric(prod2$value),4)
...
use3=use3[use3$FUEL=='E2' | use3$FUEL=='E1',]
use3 = use3 |> 
  group_by(scen,YEAR) |>
  summarise(value = sum(value))

demand$value = demand$value + use3$value
{
  x11()
